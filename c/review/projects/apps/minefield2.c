...
// acikanKare : 100 tane karemiz var hepsi açılmıssa oyun biter.
// nerdeyim : acilacaklar dizisinde eklenen son kareye kadar açmamış gerekiyor.Nerdeyim ise dizide o an nerede oldugumuz bilgisini tutuyor.
...
int main(){					
	...
	// Komsu olunan mayın sayısını hesapla ve matrise yazdır
	for( i=1; i<11; i++){
		for( j=1; j<11; j++ ){
			if( field[i][j] != 9 ){
				numberOfMine = 0;
		...
		switch(secim){
			case 1 : 
				...
				if( field[userRow][userCol] == 9 ){ 
					...
					acilanKare = 100; // Oyunu bitir, diğer durum
				}  	
				// Eğer mayın değilse ve sıfırdan farklı ise tek kare aciyoruz.
					...
					// Secilen karenin mayına komsulugu yokmus demek ki
